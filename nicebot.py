...
def new_navigate(ship, destination):
    ...
    if tuple_diff[1] == tuple_diff[0] or Direction.West not in game_map.get_unsafe_moves(ship.position.directional_offset(Direction.East),ship.position.directional_offset(Direction.West)):
        for direction in game_map.get_unsafe_moves(ship.position, destination):
            target_pos = ship.position.directional_offset(direction)
            #if not game_map[target_pos].is_occupied:
            if target_pos not in ship_next_position.values():
        ...
        if tuple_diff[1] > 0:
            ...
            if target_pos in ship_next_position.values():
                game_map[ship.position].mark_unsafe(ship)
                #logging.info("{} marked unsafe".format(ship.position))
...
global ship_next_position
ship_next_position = {}

directions = [Direction.North,Direction.East,Direction.South,Direction.West,Direction.Still]
...
while True:
    ...
    for ship in ship_status.keys():
        shipidlist = (tempship.id for tempship in me.get_ships())
        if ship not in shipidlist and ship in ship_next_position:
            logging.info("destroyed ship")
            logging.info(ship_next_position[ship])
            del ship_next_position[ship]
    ...
    for ship in me.get_ships():
        ...
        if ship.id not in ship_next_position:
            ship_next_position[ship.id] = ship.position
        ...
        if ship_status[ship.id] == "returning":
            if ship.position == me.shipyard.position:
                ...
                ship_next_position[ship.id] = ship.position.directional_offset(move)
                continue
        ...
        if ship_status[ship.id] == "exploring":
            ...
            if ship_movement_counter[ship.id] >= 2 and game_map[ship.position].halite_amount <= 9:
                for direction in directions:
                    if ship.position.directional_offset(direction) not in ship_next_position.values():
                        command_queue.append(
                            ship.move(direction))
                        ship_next_position[ship.id] = ship.position.directional_offset(direction)
                        break
                #logging.info("ship stuck")    
            elif game_map[ship.position].halite_amount >= 9:
                command_queue.append(ship.stay_still())
                ship_next_position[ship.id] = ship.position
                ...
                movement = new_navigate(ship, Position(max_halite_square[0]+ship.id, max_halite_square[1]+ship.id))
                command_queue.append(
                    ship.move(
                        movement))
                ship_next_position[ship.id] = ship.position.directional_offset(movement)
            else:
                movement = new_navigate(ship, Position(max_halite_square[0], max_halite_square[1]))
                command_queue.append(
                    ship.move(
                        movement))
                ship_next_position[ship.id] = ship.position.directional_offset(movement)
        ...
        #logging.info("Ship {} moving to {}".format(ship.id, ship_next_position[ship.id]))
        #logging.info(ship_next_position.values())
